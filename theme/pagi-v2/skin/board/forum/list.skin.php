...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <?php 
    if(isset($_GET['sca']) || empty($category_option)){
    ?>
    <nav class="bo_cate_title">
        <h1>
            <?php 
            if(isset($_GET['sca'])){
                echo htmlspecialchars(strip_tags($_GET['sca']));    
            }else{
            ?>
    ...
    <?php
        ...
        if(!empty($category_option)){
            $ca_html = str_get_html($category_option);
            $ca_list = $ca_html->find('.ca_item');
            foreach($ca_list as $ca_item) {
                ?>
                    <nav class="bo_cate_title">
                        <h1>
                            <?php echo $ca_item; ?>
                ...
                if($ca_item->plaintext == "자유갤러리"){
                    echo latest_category('theme/pic_only', $bo_table, 8, 200, 1, $ca_item->plaintext);    
                }else if($ca_item->plaintext == "공지사항" || $ca_item->plaintext == "자료실"  || $ca_item->plaintext == "자유게시판"){
                    echo latest_category('theme/title_date', $bo_table, 5, 200, 1, $ca_item->plaintext);    
                }else{
                    echo latest_category('theme/pic_title_desc', $bo_table, 3, 200, 1, $ca_item->plaintext);    
                }
